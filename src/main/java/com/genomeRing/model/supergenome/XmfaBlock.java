...
public class XmfaBlock 
{
	...
	public void createSubBlocks(int minGapLength, boolean createSubBlocks)
	{
		...
		{
			breaks.put(id, seqs.get(id).getBreakPoints(minGapLength, createSubBlocks));
		}
		
		int lastBreak = 0;
		int nextBreak = Integer.MAX_VALUE;
		Set<String> contSet;
		...
		while(true)
		{
			...
			for(String id:breaks.keySet())
				for(int b:breaks.get(id))
					if(Math.abs(b) >= lastBreak)
					{
						...
						if(Math.abs(b)-lastBreak < nextBreak-lastBreak)
							nextBreak= Math.abs(b);
			...
			res.add(new SubBlock(lastBreak, nextBreak, contSet));
